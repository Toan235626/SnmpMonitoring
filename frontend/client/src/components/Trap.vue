<template>
    <div class="trap-container">
      ...
      <div v-show="activeTab === 'latest'" class="tab-content">
        ...
        <div v-if="latestTrap">
          ...
          <p><strong>SNMP Version:</strong> {{ latestTrap.snmpVersion }}</p>
          <h4>Variables:</h4>
          <table class="trap-table">
            ...
            <tbody>
              <tr v-for="(variable, index) in latestTrap.variables" :key="index">
                <td>{{ variable.oid.value.join(".") }}</td>
                <td>{{ decodeVariableValue(variable) }}</td>
                <td>{{ variable.variable.syntaxString }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <table class="trap-table">
          ...
          <tbody>
            <tr v-for="(trap, index) in trapHistory" :key="index">
              <td>{{ index + 1 }}</td>
              <td>{{ formatSysUpTime(trap.variables[0].variable.value) }}</td>
              <td>{{ trap.variables[1].variable.value.join(".") }}</td>
              <td>{{ decodeVariableValue(trap.variables[2]) }}</td>
              <td>{{ trap.snmpVersion }}</td>
              <td>
                <button @click="showTrapDetails(trap)">View Details</button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div v-if="selectedTrap" class="modal">
        <div class="modal-content">
          ...
          <p><strong>SNMP Version:</strong> {{ selectedTrap.snmpVersion }}</p>
          <h4>Variables:</h4>
          <table class="trap-table">
            <thead>
              <tr>
  ...
  export default {
    ...
    methods: {
      async fetchTraps() {
        try {
          ...
          if (traps.length > 0) {
            ...
            if (!this.latestTrap || this.latestTrap.rawPdu !== newLatestTrap.rawPdu) {
            ...
            traps.forEach((trap) => {
              if (!this.trapHistory.some((t) => t.rawPdu === trap.rawPdu)) {
                this.trapHistory.unshift({ ...trap, timestamp: new Date() });
              }
            });
          }
        } catch (error) {
      ...
      decodeVariableValue(variable) {
        if (
          variable.variable.syntaxString === "OCTET STRING" &&
          variable.variable.printable
        ) {
          return atob(variable.variable.value);
        }
        return variable.variable.value.toString();
      },
      formatSysUpTime(ticks) {
        const seconds = Math.floor(ticks / 100);
        const days = Math.floor(seconds / (24 * 3600));
        const remainder = seconds % (24 * 3600);
        const hours = Math.floor(remainder / 3600);
        const minutes = Math.floor((remainder % 3600) / 60);
        const secs = remainder % 60;
        return `${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:${secs.toString().padStart(2, "0")}.${ticks % 100}`;
      },
      formatTimestamp(timestamp) {
        return new Date(timestamp).toLocaleString();
      },
      truncateRawPdu(rawPdu) {
        return rawPdu.length > 50 ? rawPdu.substring(0, 50) + "..." : rawPdu;
      },
